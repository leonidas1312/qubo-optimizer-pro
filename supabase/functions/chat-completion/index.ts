...
const SOLVER_SYSTEM_MESSAGE = (fileContent?: string) => `You are a specialized AI for analyzing and adapting solver code.
...
serve(async (req) => {
  ...
  try {
    const { messages, command, fileContent } = await req.json();
    ...
    if (command === 'ADD_SOLVER') {
      systemMessage = SOLVER_SYSTEM_MESSAGE(fileContent);
    }
